...
export class TokenService {
     ...
     */
    verify(taskId, command, token, expiresSec) {
     ...
     */
    async consume(token, taskId) {
        const consumedAt = Date.now();
        // Write to SQLite (synchronous better-sqlite3 call, wrapped in async for interface consistency)
        const stmt = this.db.prepare('INSERT OR IGNORE INTO consumed_tokens (token, task_id, consumed_at) VALUES (?, ?, ?)');
     ...
     */
    async loadConsumed() {
        const rows = this.db.prepare('SELECT token FROM consumed_tokens').all();
    ...
    // Private helpers
    // ---------------------------------------------------------------------------
    _computeHmac(taskId, command, expiresSec) {
