...
export class Notifier {
    ...
    async _postWebhook(task, result) {
        const { webhookUrl, sessionKey } = this.config.nova;
        const stdoutSnippet = (result.stdoutSnippet ?? '').slice(0, NOTIFY_SNIPPET_LIMIT);
        const body = JSON.stringify({
            sessionKey,
            message: `Task ${task.taskId} completed with status ${result.status}`,
            taskId: task.taskId,
            status: result.status,
            exitCode: result.exitCode,
            stdoutSnippet,
        ...
        try {
            const statusCode = await this._httpPost(webhookUrl, body, this.webhookTimeoutMs);
    ...
    async _sendFeishuNotification(task, result) {
        ...
        try {
            ...
            const sendResult = await this._httpPostJson('https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=open_id', msgBody, 5000, { Authorization: `Bearer ${tenantToken}` });
