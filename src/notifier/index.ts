...
export class Notifier {
   ...
   */
  async notify(task: Task, result: TaskResult): Promise<void> {
  ...
  private _sendViaUds(task: Task, result: TaskResult): void {
    ...
    if (sent) {
      ...
      log.warn(
        { taskId: task.taskId },
        'UDS send failed (connection dropped); falling back to webhook',
      );
  ...
  private async _sendViaWebhookWithFallback(task: Task, result: TaskResult): Promise<void> {
   ...
   */
  async _postWebhook(task: Task, result: TaskResult): Promise<boolean> {
    ...
    try {
      ...
      log.warn(
        { taskId: task.taskId, statusCode },
        'Webhook returned non-2xx; falling back to Feishu',
      );
      return false;
    } catch (err) {
   ...
   */
  async _sendFeishuNotification(task: Task, result: TaskResult): Promise<void> {
    const { appId, appSecret, bossChatId, enabled } = this.config.feishu as {
      appId: string;
      appSecret: string;
      bossChatId?: string;
      enabled?: boolean;
    ...
    if (!appId || !appSecret) {
      log.error(
        { taskId: task.taskId },
        'Feishu Bot token (appId/appSecret) not configured; cannot send fallback notification',
      );
      return;
    }
    ...
    if (!bossChatId) {
      log.error(
        { taskId: task.taskId },
        'feishu.bossChatId not configured; cannot send fallback notification',
      );
      return;
    }
    ...
    try {
      ...
      const tokenJson = await this._httpPostJson(
        'https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal',
        tokenBody,
        5000,
      );

      const tenantToken = (tokenJson as Record<string, unknown>)['tenant_access_token'];
      if (!tenantToken || typeof tenantToken !== 'string') {
        log.error(
          { taskId: task.taskId, tokenJson },
      ...
      const stdoutSnippet = (result.stdoutSnippet ?? '').slice(0, NOTIFY_SNIPPET_LIMIT);
      const msgText =
        `[root-daemon] Task result notification\n` +
        `taskId: ${task.taskId}\n` +
        `status: ${result.status}\n` +
      ...
      const sendResult = await this._httpPostJson(
   ...
   */
  _httpPost(url: string, body: string, timeoutMs: number, bearerToken?: string): Promise<number> {
