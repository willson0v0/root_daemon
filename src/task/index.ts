...
export class TaskManager {
   ...
   */
  submit(payload: SubmitPayload, preAssignedTaskId?: string): { taskId: string; approvalLink: string; expiresAt: number } {
    let taskId: string;
    ...
    if (preAssignedTaskId) {
      ...
      const existing = this.db.prepare('SELECT task_id FROM tasks WHERE task_id = ?').get(taskId) as { task_id: string } | undefined;
      if (existing) {
        skipInsert = true;
